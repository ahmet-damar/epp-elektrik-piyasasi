...
SCHEMA_PATHS = [ROOT / "db" / "schema.sql", ROOT / "supabase" / "migrations" / "20260819_0002_rls_roles.sql"]
...
def main() -> int:
    ...
    assert_contains(combined, "Service role bypasses RLS by design", "service_role_note")

    if "user_metadata" in combined:
        raise AssertionError("user_metadata must not be used as the role source")

    if "DELETE" in combined and "audit_log" in combined and "DROP POLICY IF EXISTS" not in combined:
