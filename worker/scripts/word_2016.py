...
def t2_oku(tbl, tarih_id: int) -> pd.DataFrame:
    """T2-karşılığı: Lisanslı Elektrik Üretimi, kaynak bazında.

    **Bulgu O'nun en önemli bulgusu: 2016'nın T2'si TÜM diğer yıllardan
    (2017-2025) YAPISAL OLARAK FARKLI** — dönemler-arası-karşılaştırma
    formatında (6 kolon, önceki yıl+hedef yıl) DEĞİL, TEK-DÖNEM 3-kolonlu
    format: `Kaynak Türü | Üretim Miktarı (MWh) | Oran (%)`. Önceki yılla
    karşılaştırma YOK, seçilecek "hedef dönem kolonu" da YOK — tablonun
    TAMAMI zaten hedef ayın verisi, `hedef_donem_kolonu_bul()` burada
    KULLANILAMAZ (diğer yılların `t2_oku()`'sünden kasıtlı olarak FARKLI
    imza — `hedef_ay_yil` parametresi YOK). Veri satırları `tbl.rows[1:]`
    (TEK başlık satırı, `tbl.rows[2:]` DEĞİL).

    Bulgu N (2017/2018/2019'dan taşındı — YİNE toplama gerekiyor, YENİ
    alias GEREKMİYOR): Hidrolik "AKARSU"+"BARAJLI" diye İKİ AYRI satıra
    bölünmüş (2016'da â'sız kısa "BARAJLI" — "BARAJLI HİDROLİK" DEĞİL —
    ama worker/parser.py:KAYNAK_ESLEME'nin kendi "Barajlı" alias'ı ZATEN
    tanıyor). `dict` biriktiricisiyle TEK "Hidrolik" satırına toplanıyor
    (fact_uretim_kaynak_geneli'nin UNIQUE kısıtı aksi halde ihlal edilir).
    12 ayın TAMAMI (Ocak-Aralık) gerçek dosyaya karşı doğrulandı."""
    baslik_satir = [c.text.strip() for c in tbl.rows[0].cells]
    # Başlık büyük/küçük harf ay ay değişiyor (örn. Haziran/Temmuz/Ağustos/
    # Aralık "ÜRETİM (MWh)" tüm-büyük, diğer aylar "Üretim Miktarı (MWh)"
    # title-case) — normalize_label() ile Türkçe-güvenli karşılaştırma.
    if len(baslik_satir) < 2 or "URETIM" not in normalize_label(baslik_satir[1]):
        raise ValueError(
            f"T2: beklenen 'Üretim' kolonu bulunamadı, başlık={baslik_satir}"
        )
    ...
    for row in tbl.rows[1:]:
        ...
        if kaynak_ham.strip() in ("Genel Toplam", "Toplam"):
            genel_toplam_deger = parse_sayi(hucreler[1])
            continue
        kaynak = kaynak_esle_zorunlu(kaynak_ham)
        if kaynak is None:
            continue
        deger = parse_sayi(hucreler[1])
...
def t3_oku(tbl, tarih_id: int) -> pd.DataFrame:
    """T3-karşılığı: Lisanslı Elektrik Üretimi, il bazında, iki-sütunlu
    sayfa düzeninde (Bulgu F) — diğer yıllarla AYNI yapı (`iki_blokta_il_
    degerlerini_oku()` başlık metnine değil pozisyona güveniyor, bu yüzden
    2016'nın "İL"/"İLLER" başlık salınımından ETKİLENMEZ). Eksik iller
    `uretim_mwh=0.0` ile AÇIKÇA tamamlanır, katı '81 il' assertion'ı YOK —
    asıl güvence kendi Genel Toplam'ıyla aritmetik tutarlılıktır. İl
    sayısı ay ay değişti (77-80, gerçek dosyayla doğrulandı — established
    Bulgu G deseniyle tutarlı)."""
...
def isle_ay_ulke_geneli(
    ...
    *,
    ...
    zinciri, zaten onaylı fact_tuketim batch'lerine DOKUNMAZ. T11
    tablosu BURADA YENİDEN bulunur (aynı icerir/etiket, fact_tuketim
    yazımıyla hiç etkileşmez) ve worker/scripts/word_ortak.py:
    genel_toplam_satirini_oku() ile (t11_oku() ile AYNI grup_esle_zorunlu
    + parse_sayi) Genel Toplam satırından TÜM gruplar (Sanayi DAHİL)
    okunur. Karar 2 DEĞİŞMEDİ - fact_tuketim'e hiçbir şey yazılmaz."""
    dosya_adi = MANIFEST_2016[ay]
    yol = klasor / dosya_adi
    yil = 2016
    tarih_id = yil * 100 + ay
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2016-ulke-geneli-v1"
...
def isle_ay_uretim_geneli(
    ...
    *,
    ...
    """`fact_uretim_kaynak_geneli` + `fact_uretim_il_geneli`'ni 2016 Word
    aylarına doldurur — desende `word_2017.py:isle_ay_uretim_geneli()` ile
    AYNI, ama `t2_oku()` çağrısı `hedef_ay_yil` almıyor (Bulgu O — 2016
    tek-dönem format, dönem seçimi YOK).

    **Arama metni — Ocak/Şubat'ta "Lisanslı" kelimesi YOK (Bulgu O):**
    diğer yılların `icerir=["Lisanslı Elektrik Üretiminin ..."]` arama
    metni burada "Lisanslı" ÇIKARILARAK kullanılıyor (`icerir=["Elektrik
    Üretiminin Kaynak/İl Bazında Dağılımı"]`) — bu, Mart-Aralık'ın
    "Lisanslı"LI başlıklarını da (alt-dize olarak) YAKALAR, hem Ocak/
    Şubat hem Mart-Aralık için TEK arama metni yeterli. `icermez=
    ["Lisanssız"]` savunma amaçlı eklendi (Lisanssız kaynak/il tablolarının
    başlık metni zaten FARKLI bir ifade kullanıyor — "Kaynaklara Dağılımı"/
    "İllere Göre Dağılımı", "Kaynak Bazında"/"İl Bazında" DEĞİL — ama
    ucuz bir önlem).
    ...
    **2026-09-13 — Lisanslı (T2+T3) yüklenir, Lisanssız (T5/T6) KAPSAM
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2016-uretim-geneli-v1"
    ...
    t2_tbl, t2_baslik = tek_aday_bul(
        basliklar,
        icerir=["Elektrik Üretiminin Kaynak Bazında Dağılımı"],
        icermez=["Lisanssız"],
        etiket="T2",
    )
    ...
    t3_tbl, t3_baslik = tek_aday_bul(
        basliklar,
        icerir=["Elektrik Üretiminin İl Bazında Dağılımı"],
        icermez=["Lisanssız"],
        etiket="T3",
    )
    print(f"  T3 başlık: {t3_baslik!r}")

    kaynak_ham = t2_oku(t2_tbl, tarih_id)
    ...
    ingest.audit_log_yaz(
        ...
        payload={
            "olay": "ingest_tamamlandi",
            "tarih_id": tarih_id,
            "kaynak": "word_2016_uretim_geneli",
            "tablolar": audit_tablolar,
            "not": "Yalnız Lisanslı (T2+T3) yüklendi - Lisanssız (T5/T6) "
            "kapsam dışı işaretlendi (Bulgu L: T6 hiçbir zaman Brüt "
            "Üretim değil, İhtiyaç Fazlası Satın Alınan). T2 tek-dönem "
            "3-kolonlu bespoke format (Bulgu O) — 'AKARSU'+'BARAJLI' "
            "TOPLANARAK tek 'Hidrolik' satırına indirgendi (Bulgu N).",
        },
    )
    ...
    for tablo_adi in ("fact_uretim_kaynak_geneli", "fact_uretim_il_geneli"):
        pipeline.kapsam_disi_isaretle(
            ...
            sebep="2016'da T6 (Lisanssız, il bazında) var olsa da ÖLÇÜMLE "
