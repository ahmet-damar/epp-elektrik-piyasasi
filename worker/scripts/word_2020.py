"""EPP — 2020 Word (.docx) EPDK aylık raporları: TEK SEFERLİK tarihsel
aktarım tarifi. Bkz. dokumanlar/08_word_2016_2022_kapsam.md (teşhis) ve
worker/scripts/word_2021.py (yapısal temel — bu dosya onun kopyası, YIL
...
- **Taksonomi kararı ZATEN UYGULI** (2021/2022'de karar verildi — RENAME,
  mevsimsellik doğrulamasıyla desteklendi, dokumanlar/08) —
  "Ticarethane"/"Tarımsal Sulama" `_GRUP_TAKMA_ADLAR`'da baştan var,
  2020'nin TÜMÜ (mekanik taramayla doğrulandı, eski taksonomi) T11 için AÇIK.
- **T11'in ay/yıl doğrulama ÇAPASI FARKLI:** 2020'nin T11-karşılığı başlığı
  ("Tüketici Türü Bazında Dağılımı (MWh)") HİÇ "{Ay} {Yıl} Döneminde"
  içermiyor (2016'nın erken aylarındaki AYNI sorun, Bulgu 1) — bu yüzden
  ay/yıl doğrulaması T11 başlığından DEĞİL, dokümanın kapak paragrafından
  ("{Yıl} Yılı {Ay} Ayı Elektrik Piyasası Genel Görünümü", HER örnekte
  tutarlı bulundu) yapılıyor.
- **T10 GERÇEKTEN il-ONLY — 2020'nin TÜM 12 ayında** (2021'in aksine hiç
  il×grup geçişi YOK, Ocak VE Aralık ikisi de kontrol edildi) — grup boyutu
  kaynakta hiç yok, `isle_ay()` T11'i T10'dan BAĞIMSIZ okuyor (word_2021.py
  ile AYNI desen), T10 HER ay için `kapsam_disi_isaretle(fact_abone)` ile
  işaretlenmesi BEKLENİYOR. T11 ve T4 bundan ETKİLENMEZ.

Kapsam (2021 ile AYNI): T11-karşılığı → fact_tuketim (Sanayi HARİÇ, Karar
2), T10-karşılığı → fact_abone (Sanayi DAHİL, yapısal olarak mümkünse),
T4-karşılığı → fact_uretim (Lisanssız). T13 (Karar 1) ve T1/Lisanslı
(Karar 3) kapsam dışı — `pipeline.kapsam_disi_isaretle()` ile doğrudan
isle_ay()/isle_ay_t4() içinde işaretlenir.

Kullanım:
    python -m worker.scripts.word_2020 --ay 6                  # tek ay, gerçek yükleme
    python -m worker.scripts.word_2020                          # 2020'nin TÜMÜ
    python -m worker.scripts.word_2020 --dry-run                # yalnız parse, DB'ye YAZMA
    python -m worker.scripts.word_2020 --t4                     # T11/T10 yerine T4
"""
...
# taramasıyla bulundu. Aralık dosyası "_Content_News_" öneki taşıyor (diğer
# tüm aylar "_Content_FastAccess_") — kozmetik, dosya normal şekilde açılıyor.
MANIFEST_2020: dict[int, str] = {
    1: "_PortalAdmin_Uploads_Content_FastAccess_6e8d683636812.docx",
    2: "_PortalAdmin_Uploads_Content_FastAccess_95c84f9592248.docx",
    3: "_PortalAdmin_Uploads_Content_FastAccess_dbb6975f26171.docx",
    4: "_PortalAdmin_Uploads_Content_FastAccess_92acc29d48309.docx",
    5: "_PortalAdmin_Uploads_Content_FastAccess_e7376e2716897.docx",
    6: "_PortalAdmin_Uploads_Content_FastAccess_2c9bb70498188.docx",
    7: "_PortalAdmin_Uploads_Content_FastAccess_344dda3361939.docx",
    8: "_PortalAdmin_Uploads_Content_FastAccess_1597da3f39850.docx",
    9: "_PortalAdmin_Uploads_Content_FastAccess_4991201d95065.docx",
    10: "_PortalAdmin_Uploads_Content_FastAccess_051641c324108.docx",
    11: "_PortalAdmin_Uploads_Content_FastAccess_fdae816916948.docx",
    12: "_PortalAdmin_Uploads_Content_News_2234dc9921633.docx",
}

# Taksonomi kararı UYGULANDI (2026-09-03, madde 1/2a, dokumanlar/08) —
# RENAME olarak kabul edildi, mevsimsellik doğrulamasıyla desteklendi.
_GRUP_TAKMA_ADLAR = {
...
def grup_esle_zorunlu(metin: str) -> str | None:
    ...
    ValueError — yalnız GERÇEKTEN tanınmayan (beklenmedik) bir etiket için."""
    temiz = metin.strip()
    ...
    if grup is None:
        raise ValueError(
            ...
            "worker/scripts/word_2020.py: _GRUP_TAKMA_ADLAR'a eklenmeli mi "
...
def kaynak_esle_zorunlu(metin: str) -> str | None:
    ...
    if eslesme is None:
        raise ValueError(
            ...
            "worker/scripts/word_2020.py: _KAYNAK_TAKMA_ADLAR'a eklenmeli mi kontrol et."
        )
    return eslesme[0]


# 2021'in "Küthahya"/"AFYONK."/"K.MARAŞ" gibi tekrarlayan yazım hatalarının
# 2020'de de görülüp görülmediği dry-run ile keşfedilecek — boş başlıyor,
# YENİ bir ay YENİ bir hata getirirse buraya eklenir.
...
# 2020'de (word_2022.py'nin Nisanındaki gibi) bilinen bir ay/yıl etiket
# hatası bulunmadı — boş başlıyor.
...
def _ay_yil_dogrula_kapak(
    ...
    """T11-karşılığı başlığı ay/yıl İÇERMİYOR (bkz. modül notu) — bunun
    yerine dokümanın KAPAK paragrafından ("{Yıl} Yılı {Ay} Ayı Elektrik
    Piyasası Genel Görünümü") doğrulanır. `_BILINEN_ETIKET_HATALARI`'nda
    BELGELENMİŞ istisnalar dışında uyuşmazlıkta ValueError."""
    ...
    if bilinen == (beklenen_ay_adi, beklenen_yil):
        ...
        return
    raise ValueError(
        f"MANIFEST_2020 uyuşmazlığı! beklenen={beklenen_ay_adi} {beklenen_yil}, "
        f"kapağın kendi başlığı={bulunan_ay} {bulunan_yil} (başlık: {kapak_baslik!r})"
    )


def t11_oku(tbl, tarih_id: int) -> pd.DataFrame:
    """T11-karşılığı: wide format. Karar 2: Sanayi HARİÇ. word_2021.py:
...
def t10_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    """T10-karşılığı: uzun format. word_2022.py'nin DİNAMİK başlık-satırı
    bulma mantığıyla AYNI (2020'de başlık satırı sayısı da ay ay değişiyor,
    Ocak 1 satır / Aralık 2 satır) — 'Tüketici Türü' İÇEREN satır dinamik
    aranır. **2020'nin TÜM 12 ayında bu satır HİÇ bulunmuyor** (T10
    yapısal olarak il-ONLY, dokumanlar/08) — bu fonksiyon HER ay ValueError
    fırlatması BEKLENEREK yazıldı, isle_ay() bunu yakalayıp T10'u
    kapsam_disi işaretler."""
...
def t4_oku(tbl, tarih_id: int) -> pd.DataFrame:
    """T4-karşılığı: il×kaynak matrisi, Lisanssız. word_2021.py:t4_oku() ile
    BİREBİR AYNI mantık."""
    baslik_satir = [c.text.strip() for c in tbl.rows[0].cells]
...
def t2_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    ...
    dönemler-arası-karşılaştırma format (Bulgu B) — dönem satırı '2020
    OCAK' gibi yıl-önce/tüm-büyük (2021/2022/2023/2024/2025 ile AYNI).
    Kendi 'Genel Toplam' satırıyla aritmetik tutarlılık kontrol edilir.
    12 ayın TAMAMI GERÇEK dosyaya karşı kontrol edildi — Bulgu I'in
    (3 satırlık bölünmüş başlık) benzeri 2020'de GÖRÜLMEDİ (hepsi düz
    2-satır), yine de aynı dinamik atlama ucuz bir önlem olarak burada
    da var. Kaynak etiketleri (DOĞAL GAZ/İTHAL KÖMÜR/HİDROLİK/RÜZGAR/...)
    tüm-büyük ama â/varyant sorunu YOK (worker/scripts/word_2020.py
    dry-run taramasıyla 12 ayın TAMAMI için doğrulandı)."""
...
def t3_oku(tbl, tarih_id: int) -> pd.DataFrame:
    ...
    aritmetik tutarlılıktır. 12 ayın TAMAMI dry-run'da 79/80 dolu hücre
    verdi (son satırın ilk yarısı boş, established Kilis-benzeri desen),
    kod bunu zaten 0.0 ile tamamlıyor."""
...
def isle_ay(
    ...
    *,
    ...
    """word_2021.py:isle_ay() ile BİREBİR AYNI akış (T10 T11'den BAĞIMSIZ
    okunur) — TEK fark: ay/yıl doğrulaması kapak paragrafından yapılır."""
    dosya_adi = MANIFEST_2020[ay]
    ...
    batch_id = ingest.batch_olustur(conn, source_asset_id, "word-2020-v1", "1")
    ...
    if abone_ham is None:
        pipeline.kapsam_disi_isaretle(
            ...
            "2020'nin TÜMÜ (dokumanlar/08_word_2016_2022_kapsam.md).",
...
def isle_ay_t4(
    ...
    *,
    ...
    """T4-karşılığı (fact_uretim, Lisanssız) için AYRI batch — word_2021.py:
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2020-t4-v1"
...
def isle_ay_ulke_geneli(
    ...
    *,
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2020-ulke-geneli-v1"
...
def isle_ay_uretim_geneli(
    ...
    *,
    ...
    """`fact_uretim_kaynak_geneli` + `fact_uretim_il_geneli`'ni 2020 Word
    aylarına doldurur — `word_2021.py:isle_ay_uretim_geneli()` ile
    BİREBİR AYNI desen.

    **2026-09-13 — Lisanslı (T2+T3) yüklenir, Lisanssız (T5/T6) KAPSAM
    DIŞI:** Bulgu L (`12_word_uretim_envanteri.md`) ÖLÇÜMLE kanıtladı —
    T6 (Lisanssız, il bazında), VAR OLDUĞU HER YIL (2020 DAHİL — Bulgu
    L'nin ÖLÇÜM aralığının bir parçası, 2020 Ocak/Haziran doğrudan
    kontrol edildi) Excel'in "Brüt Lisanssız Üretim" tanımını DEĞİL, dar
    bir "İhtiyaç Fazlası Satın Alınan" alt-kümesini ölçüyor — Bulgu D'nin
    ilkesiyle tutarlı olarak KAPSAM DIŞI. Lisanssız HER İKİ tabloda da
    simetrik olarak kapsam dışı bırakılıyor, mutabakat kontrolüne
    istisna EKLENMİYOR."""
    dosya_adi = MANIFEST_2020[ay]
    yol = klasor / dosya_adi
    yil = 2020
    ...
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2020-uretim-geneli-v1"
    ...
    ingest.audit_log_yaz(
        ...
        payload={
            "olay": "ingest_tamamlandi",
            "tarih_id": tarih_id,
            "kaynak": "word_2020_uretim_geneli",
            "tablolar": audit_tablolar,
    ...
    for tablo_adi in ("fact_uretim_kaynak_geneli", "fact_uretim_il_geneli"):
        pipeline.kapsam_disi_isaretle(
            ...
            sebep="2020'de T6 (Lisanssız, il bazında) var olsa da ÖLÇÜMLE "
            "kanıtlandı ki Brüt Üretim DEĞİL, İhtiyaç Fazlası Satın Alınan "
            "ölçüyor (2020 Ocak/Haziran doğrudan, 2022 Mart-Ağustos ile "
            "birlikte doğrulandı) - bkz. 12_word_uretim_envanteri.md Bulgu L. "
            "Simetri için T5 de yüklenmiyor (2016-2017 Karar 4 ile aynı ilke).",
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="EPP: 2020 Word raporlarını yükle (tek seferlik)"
    )
    ...
    ap.add_argument("--actor", default="manual-cli:word-2020")
    ap.add_argument(
    ...
    aylar = [args.ay] if args.ay else sorted(MANIFEST_2020)
    isleyici = (
    ...
    if args.dry_run:
        for ay in aylar:
            try:
                ...
                print(f"  [BEKLEMEDE] {ingest.AY_ADLARI[ay]} 2020 dry-run'da hata: {e}")
        return 0
    ...
    with psycopg.connect(database_url, prepare_threshold=None) as conn:
        for ay in aylar:
            try:
                ...
                conn.rollback()
                print(
                    f"  [BEKLEMEDE] {ingest.AY_ADLARI[ay]} 2020 işlenirken hata oluştu, "
