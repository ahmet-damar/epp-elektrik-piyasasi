...
# 2026-09-13 (ADIM 4, T2): Nisan 2021'in T2 (Lisanslı, kaynak bazında)
# tablosu "RÜZGÂR" (inceltme işaretli â, tüm-büyük) yazıyor — diğer 11 ay
# ve T4'ün hepsi â'sız "RÜZGAR" kullanıyor. worker/parser.py:KAYNAK_
# ESLEME'nin "Rüzgar" anahtarı â'yı ASCII'ye katlamıyor (_TR_SADE'de â/Â
# yok) — worker/parser.py'ye dokunmadan (mimari karar) burada tek
# satırlık bir takma ad yeterli.
...
def t2_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    ...
    dönemler-arası-karşılaştırma format (Bulgu B) — dönem satırı '2021
    HAZİRAN' gibi yıl-önce/tüm-büyük (2022/2023/2024/2025 ile AYNI).
    Kendi 'Genel Toplam' satırıyla aritmetik tutarlılık kontrol edilir.
    12 ayın TAMAMI GERÇEK dosyaya karşı kontrol edildi — 2024'ün Bulgu
    I'i (3 satırlık bölünmüş başlık) 2021'de GÖRÜLMEDİ, yine de aynı
    dinamik atlama ucuz bir önlem olarak burada da var."""
    donem_satiri = [c.text.strip() for c in tbl.rows[0].cells]
    baslik_satiri = [c.text.strip() for c in tbl.rows[1].cells]
    hedef_kolon = hedef_donem_kolonu_bul(
...
def isle_ay_ulke_geneli(
    ...
    *,
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2021-ulke-geneli-v1"
...
def isle_ay_uretim_geneli(
    ...
    *,
    ...
    """`fact_uretim_kaynak_geneli` + `fact_uretim_il_geneli`'ni 2021 Word
    aylarına doldurur — `word_2022.py:isle_ay_uretim_geneli()` ile
    BİREBİR AYNI desen.
    ...
    **2026-09-13 — Lisanslı (T2+T3) yüklenir, Lisanssız (T5/T6) KAPSAM
    ...
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2021-uretim-geneli-v1"
    ...
    ingest.audit_log_yaz(
        ...
        payload={
            "olay": "ingest_tamamlandi",
            "tarih_id": tarih_id,
            "kaynak": "word_2021_uretim_geneli",
            "tablolar": audit_tablolar,
    ...
    for tablo_adi in ("fact_uretim_kaynak_geneli", "fact_uretim_il_geneli"):
        pipeline.kapsam_disi_isaretle(
            ...
            sebep="2021'de T6 (Lisanssız, il bazında) var olsa da ÖLÇÜMLE "
            "kanıtlandı ki Brüt Üretim DEĞİL, İhtiyaç Fazlası Satın Alınan "
            "ölçüyor (bkz. 12_word_uretim_envanteri.md Bulgu L). Simetri "
            "için T5 de yüklenmiyor (2016-2017 Karar 4 ile aynı ilke).",
