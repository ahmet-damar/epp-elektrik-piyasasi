"""EPP — 2022 Word (.docx) EPDK aylık raporları: TEK SEFERLİK tarihsel
aktarım tarifi. Bkz. dokumanlar/08_word_2016_2022_kapsam.md (teşhis) ve
worker/scripts/word_2023.py (2023'ün — 2022'yle AYNI iki-şablon deseni
taşıyan — tarifi; bu dosya onun yapısal bir kopyası, YIL BAZLI AYRI TARİF
ilkesi bilinçli tercih).
...
fact_uretim (Lisanssız). T13 (Karar 1) ve T1/Lisanslı (Karar 3) kapsam
dışı — HER ay için `pipeline.kapsam_disi_isaretle()` ile açıkça işaretlenir
(2023-2025'te bu ayrı bir turda yapılmıştı; 2022'den itibaren doğrudan
isle_ay()/isle_ay_t4() içine taşındı, aynı ay içinde tek adımda).

**Bulgu — Nisan 2022 dosyasının T11 tablosu YANLIŞ ETİKETLİ (dokumanlar/08
Bulgu 6 devamı):** Nisan dosyasının kapak başlığı ve TÜM diğer tabloları
"Nisan 2022" diyor, ama T11-karşılığı (Tablo 2.6) tablosunun kendi başlığı
"Mart 2022 Döneminde" yazıyor. Bu bir veri DUPLIKASYONU DEĞİL — Mart
dosyasının kendi Genel Toplam'ı (21.565.551,4 MWh) ile Nisan dosyasının bu
tablosunun Genel Toplam'ı (19.854.983,16 MWh) TAMAMEN FARKLI, yani veri
muhtemelen gerçekten Nisan'a ait, yalnız EPDK'nın kendi başlık metni bir
önceki ayın metnini KOPYALAYIP GÜNCELLEMEMİŞ (kopyala-yapıştır kalıntısı).
`_BILINEN_ETIKET_HATALARI` bu TEK istisnayı açıkça belgeliyor — sessizce
GENELLENMEZ, başka bir ay/yıl aynı sorunu YENİDEN yaşarsa ayrı ele alınır.

Kullanım:
    python -m worker.scripts.word_2022 --ay 5                  # tek ay, gerçek yükleme
    python -m worker.scripts.word_2022                          # 2022'nin TÜMÜ
    python -m worker.scripts.word_2022 --dry-run                # yalnız parse, DB'ye YAZMA
    python -m worker.scripts.word_2022 --t4                     # T11/T10 yerine T4
"""
...
# ay(int) -> dosya adı. dokumanlar/08_word_2016_2022_kapsam.md Bulgu 0'daki
# envanter taramasıyla bulundu; her giriş isle_ay() içinde kendi
# başlığından yeniden doğrulanır (bkz. _ay_yil_dogrula, _BILINEN_ETIKET_HATALARI).
MANIFEST_2022: dict[int, str] = {
    1: "_PortalAdmin_Uploads_Content_FastAccess_deda584e94213.docx",
    2: "_PortalAdmin_Uploads_Content_FastAccess_b7a2797582841.docx",
    3: "_PortalAdmin_Uploads_Content_FastAccess_fc407bed16857.docx",
    4: "_PortalAdmin_Uploads_Content_FastAccess_7429911590338.docx",
    5: "_PortalAdmin_Uploads_Content_FastAccess_29153ef590799.docx",
    6: "_PortalAdmin_Uploads_Content_FastAccess_ba1fef9a31902.docx",
    7: "_PortalAdmin_Uploads_Content_FastAccess_d274971b38299.docx",
    8: "_PortalAdmin_Uploads_Content_FastAccess_dd8e572c75338.docx",
    9: "_PortalAdmin_Uploads_Content_FastAccess_3d8a81a712090.docx",
    10: "_PortalAdmin_Uploads_Content_FastAccess_5128c0d858840.docx",
    11: "_PortalAdmin_Uploads_Content_FastAccess_2df0321866923.docx",
    12: "_PortalAdmin_Uploads_Content_FastAccess_fa7155f815913.docx",
}
...
def kaynak_esle_zorunlu(metin: str) -> str | None:
    ...
    if eslesme is None:
        raise ValueError(
            ...
            "worker/scripts/word_2022.py: _KAYNAK_TAKMA_ADLAR'a eklenmeli mi kontrol et."
        )
    return eslesme[0]


# Ocak/Şubat 2022'nin T4-karşılığı tablosunda "Kütahya" yerine "Küthahya"
# yazılmış (fazladan bir 'h', kod sayfası/encoding sorunu DEĞİL — 'ü' doğru
# kodlanmış U+00FC, yalnız gerçek bir kaynak yazım hatası). Genel bir
# normalize_label() düzeltmesi DEĞİL (worker/parser.py'a dokunulmadı) —
# yalnız bu tarife özel, belgelenmiş bir istisna.
...
# Bkz. modül notu — Nisan 2022'nin T11 tablosu kendi başlığında "Mart 2022"
# yazıyor ama kapak+diğer tablolar "Nisan 2022" diyor, ve veri Mart
# dosyasının Genel Toplam'ından FARKLI (duplikasyon değil, etiket hatası).
# (ay, yil, tablo_etiketi) -> gerçek beklenen (ay_adi, yil) override.
_BILINEN_ETIKET_HATALARI: dict[tuple[int, int, str], tuple[str, int]] = {
    (4, 2022, "T11"): ("Nisan", 2022),  # kaynağın kendi metni "Mart 2022" diyor, YANLIŞ
}
...
def _ay_yil_dogrula(
    ...
    """T11-karşılığı başlığından ay/yıl çıkarır, MANIFEST_2022'deki
    beklenenle karşılaştırır. `_BILINEN_ETIKET_HATALARI`'nda BELGELENMİŞ
    istisnalar dışında uyuşmazlıkta ValueError (bkz. word_2023.py'nin AYNI
    fonksiyonu — davranış birebir aynı, yalnız istisna mekanizması eklendi)."""
    ...
    if bilinen == (beklenen_ay_adi, beklenen_yil):
        print(
            ...
            f"diyor ama MANIFEST_2022 + dosyanın DİĞER tabloları {beklenen_ay_adi} "
            f"{beklenen_yil} diyor — dokumanlar/08 Bulgu 6'da belgelendi, veri "
            "duplikasyonu DEĞİL (Genel Toplam farklı). Devam ediliyor."
        )
        return
    raise ValueError(
        f"{etiket}: MANIFEST_2022 uyuşmazlığı! beklenen={beklenen_ay_adi} "
...
def t10_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    ...
    **Başlık satırı SAYISI da sabit değil (2026-09-03, gece-boyu turu, madde
    ...
    for row in tbl.rows[baslik_satir_idx + 1 :]:
        ...
        # satırlarında Word tablosunun kendi hücre-birleştirmesi yüzünden
        # BOŞ — 05_kaynak_dosya_sozlesmesi.md'deki T13 "ileri doldurma"
        # ilkesiyle AYNI: boşsa bir önceki dolu İl Adı'nı kullan.
        il_adi_ham = _il_adi_temizle(hucreler[0])
        if il_adi_ham:
...
def t4_oku(tbl, tarih_id: int) -> pd.DataFrame:
    """T4-karşılığı: il×kaynak matrisi, Lisanssız. bkz. word_2023.py:t4_oku()
    docstring'i — aynı mantık (eksik iller AÇIKÇA 0, Genel Toplam ile
    aritmetik tutarlılık doğrulaması)."""
    baslik_satir = [c.text.strip() for c in tbl.rows[0].cells]
...
def t2_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    ...
    dönemler-arası-karşılaştırma format (Bulgu B) — dönem satırı '2022
    HAZİRAN' gibi yıl-önce/tüm-büyük (2023/2024/2025 ile AYNI). Kendi
    'Genel Toplam' satırıyla aritmetik tutarlılık kontrol edilir. 12
    ayın TAMAMI GERÇEK dosyaya karşı kontrol edildi — 2024'ün Bulgu I'i
    (3 satırlık bölünmüş başlık) 2022'de GÖRÜLMEDİ (hepsi düz 2-satır),
    yine de aynı dinamik atlama ucuz bir önlem olarak burada da var."""
        ...
        satirlar.append(
            {
                ...
                "kaynak": kaynak,
                "lisans": "Lisanslı",
                "uretim_mwh": parse_sayi(hucreler[hedef_kolon]),
            }
        )
...
def isle_ay(
    ...
    *,
    ...
    """word_2023.py:isle_ay() ile BİREBİR AYNI akış — TEK fark: bu ay için
    ayrıca `pipeline.kapsam_disi_isaretle()` ile T13'ü (Karar 1) açıkça
    işaretler (2023-2025'te bu ayrı bir turda yapılmıştı)."""
    dosya_adi = MANIFEST_2022[ay]
    ...
    # dokumanlar/08 Bulgu 6: Ocak-Nisan başlığı "İl Bazında" (grup boyutu
    # METİNDE yok), Mayıs-Aralık "İl ve Tüketici Türü Bazında" — ikisi de
    # yakalanacak şekilde yalnız "İl" arandı (yapı zaten doğrulandı, bkz.
    # t10_oku() docstring'i).
    ...
    batch_id = ingest.batch_olustur(conn, source_asset_id, "word-2022-v1", "1")
    ...
    # Karar 1 — T13 (fact_serbest_tuketici) kaynakta yok, açıkça işaretle.
...
def isle_ay_t4(
    ...
    *,
    ...
    """T4-karşılığı (fact_uretim, Lisanssız) için AYRI batch — word_2023.py:
    isle_ay_t4() ile AYNI mantık + Karar 3 (T1) kapsam_disi işaretlemesi
    eklendi. Tüketici grubu taksonomi sorunundan BAĞIMSIZ (kaynak
    türlerini kullanır, tüketici grubu DEĞİL) — Ocak-Nisan dahil TÜM 12 ay
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2022-t4-v1"
    ...
    # Karar 3 — T1 (Lisanslı kesiti) kaynakta yok, açıkça işaretle.
    pipeline.kapsam_disi_isaretle(
        conn,
        tarih_id=tarih_id,
...
def isle_ay_ulke_geneli(
    ...
    *,
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2022-ulke-geneli-v1"
...
def isle_ay_uretim_geneli(
    ...
    *,
    ...
    """`fact_uretim_kaynak_geneli` + `fact_uretim_il_geneli`'ni 2022 Word
    aylarına doldurur — `word_2023.py:isle_ay_uretim_geneli()` ile
    BİREBİR AYNI desen.

    **2026-09-13 — Lisanslı (T2+T3) yüklenir, Lisanssız (T5/T6) KAPSAM
    DIŞI:** Bulgu L (`12_word_uretim_envanteri.md`) ÖLÇÜMLE kanıtladı —
    T6 (Lisanssız, il bazında), VAR OLDUĞU HER YIL (2022 dahil, hem
    Haziran'daki başlık değişikliğinden ÖNCE hem SONRA) Excel'in "Brüt
    ...
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2022-uretim-geneli-v1"
    ...
    for tablo_adi in ("fact_uretim_kaynak_geneli", "fact_uretim_il_geneli"):
        pipeline.kapsam_disi_isaretle(
            ...
            sebep="2022'de T6 (Lisanssız, il bazında) var olsa da ÖLÇÜMLE "
            "kanıtlandı ki Brüt Üretim DEĞİL, İhtiyaç Fazlası Satın Alınan "
            "ölçüyor (2022 Mart-Ağustos VE 2020 Ocak/Haziran'da doğrulandı) "
            "- bkz. 12_word_uretim_envanteri.md Bulgu L. Simetri için T5 "
            "de yüklenmiyor (2016-2017 Karar 4 ile aynı ilke).",
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="EPP: 2022 Word raporlarını yükle (tek seferlik)"
    )
    ...
    ap.add_argument("--actor", default="manual-cli:word-2022")
    ap.add_argument(
    ...
    # KESİN KURAL (2026-09-03): --onayla YOK, BİLİNÇLİ OLARAK — bu script
    # hiçbir batch'i asla aktive etmez, ne CLI bayrağıyla ne başka türlü.
    args = ap.parse_args()

    aylar = [args.ay] if args.ay else sorted(MANIFEST_2022)
    isleyici = (
    ...
    if args.dry_run:
        for ay in aylar:
            try:
                ...
                print(f"  [BEKLEMEDE] {ingest.AY_ADLARI[ay]} 2022 dry-run'da hata: {e}")
        return 0
    ...
    with psycopg.connect(database_url, prepare_threshold=None) as conn:
        for ay in aylar:
            try:
                ...
                conn.rollback()
                print(
                    f"  [BEKLEMEDE] {ingest.AY_ADLARI[ay]} 2022 işlenirken hata oluştu, "
