"""EPP — 2018 Word (.docx) EPDK aylık raporları: TEK SEFERLİK tarihsel
...
- **2019'da bulunan T4 "Güneş" iki-kolon sorunu (Fotovoltaik/Yoğunlş.,
  ...
  2018'de aynı sürprizlerin çıkıp çıkmadığı dry-run ile doğrulanacak,
  çıkmazsa bu kod zararsız kalır (kullanılmayan bir dal).

Kullanım:
    python -m worker.scripts.word_2018 --ay 6                  # tek ay, gerçek yükleme
    python -m worker.scripts.word_2018                          # 2018'un TÜMÜ
    python -m worker.scripts.word_2018 --dry-run                # yalnız parse, DB'ye YAZMA
    python -m worker.scripts.word_2018 --t4                     # T11/T10 yerine T4
"""
...
# taramasıyla bulundu (2018'in TÜMÜ "_Content_FastAccess_" öneki taşıyor).
MANIFEST_2018: dict[int, str] = {
    1: "_PortalAdmin_Uploads_Content_FastAccess_df805e4a21013.docx",
    2: "_PortalAdmin_Uploads_Content_FastAccess_c9a2bc0866462.docx",
    3: "_PortalAdmin_Uploads_Content_FastAccess_9c15059c29022.docx",
    4: "_PortalAdmin_Uploads_Content_FastAccess_2ffab71671039.docx",
    5: "_PortalAdmin_Uploads_Content_FastAccess_aedcb0fe55792.docx",
    6: "_PortalAdmin_Uploads_Content_FastAccess_fc5922ea19020.docx",
    7: "_PortalAdmin_Uploads_Content_FastAccess_4f2149af79123.docx",
    8: "_PortalAdmin_Uploads_Content_FastAccess_77b90b4c49319.docx",
    9: "_PortalAdmin_Uploads_Content_FastAccess_d2fae89149445.docx",
    10: "_PortalAdmin_Uploads_Content_FastAccess_e66718fe73386.docx",
    11: "_PortalAdmin_Uploads_Content_FastAccess_36ef1b0b51970.docx",
    12: "_PortalAdmin_Uploads_Content_FastAccess_3d171e4060561.docx",
}
...
def grup_esle_zorunlu(metin: str) -> str | None:
    ...
    if grup is None:
        raise ValueError(
            ...
            "worker/scripts/word_2018.py: _GRUP_TAKMA_ADLAR'a eklenmeli mi "
            "kontrol et (yeni bir ay yeni bir kısaltma/varyant getirmiş olabilir)."
        )
    return grup


# dokumanlar/08 devamı — 2018'in T4 tablosunda "Güneş" iki AYRI kolona
...
def kaynak_esle_zorunlu(metin: str) -> str | None:
    ...
    if eslesme is None:
        raise ValueError(
            ...
            "worker/scripts/word_2018.py: _KAYNAK_TAKMA_ADLAR'a eklenmeli mi kontrol et."
        )
    return eslesme[0]
...
# dokumanlar/08 devamı — Ağustos/Eylül 2018'in T4 tablosunda "BOŞ-VERİ-
# ŞEHİR" adlı, hiçbir gerçek il_kodu'na eşlenemeyen FAZLADAN bir satır var
# (bkz. t4_oku()'daki kullanım yeri için detaylı not). Genel bir _IL_ADI_
# DUZELT girişi DEĞİL — bu satır bir yazım hatası değil, gerçekten
# tanımlanamayan bir kaynak anomalisi, bu yüzden AÇIKÇA atlanıyor.
_BILINEN_ANOMALI_SATIRLAR = {"BOŞ-VERİ-ŞEHİR"}
...
def _il_adi_temizle(il_adi_ham: str) -> str:
    ...
    # taşındı, 2018'de aynı sorun çıkmasa da zararsız.
...
def _ay_yil_dogrula_kapak(
    ...
    if bilinen == (beklenen_ay_adi, beklenen_yil):
        ...
        return
    raise ValueError(
        f"MANIFEST_2018 uyuşmazlığı! beklenen={beklenen_ay_adi} {beklenen_yil}, "
...
def t4_oku(tbl, tarih_id: int) -> pd.DataFrame:
    ...
    for row in tbl.rows[1:]:
        ...
        if il_adi_ham in _BILINEN_ANOMALI_SATIRLAR:
            # dokumanlar/08 devamı — Ağustos/Eylül 2018'de EPDK'nın kendi
            # tablosunda "BOŞ-VERİ-ŞEHİR" adlı, hiçbir gerçek il_kodu'na
            # eşlenemeyen FAZLADAN bir satır var (81 gerçek ilin YANINDA,
            # onların YERİNE DEĞİL — örn. Ağustos'ta BURDUR AYRICA
            # mevcut). Küçük bir gerçek değer taşıyor (~3,3 MW) ama HANGİ
            # ile ait olduğu belirlenemiyor — TAHMİN ETMEK yerine bu tek
            # satır AÇIKÇA atlanıyor; Genel Toplam'ın kendi toleransı
            # (~%0,1) bu farkı zaten kapsıyor (doğrulandı: Ağustos'ta fark
            # 3,34 MW < tolerans 4,96 MW).
            print(
...
def isle_ay(
    ...
    *,
    ...
    batch_id = ingest.batch_olustur(conn, source_asset_id, "word-2018-v1", "1")
...
def isle_ay_t4(
    ...
    *,
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2018-t4-v1"
...
def isle_ay_ulke_geneli(
    ...
    *,
    ...
    ay_adi = ingest.AY_ADLARI[ay]
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2018-ulke-geneli-v1"
...
def main() -> int:
    ...
    ap.add_argument("--actor", default="manual-cli:word-2018")
    ap.add_argument(
    ...
    if args.dry_run:
        for ay in aylar:
            try:
                ...
                print(f"  [BEKLEMEDE] {ingest.AY_ADLARI[ay]} 2018 dry-run'da hata: {e}")
        return 0
    ...
    with psycopg.connect(database_url, prepare_threshold=None) as conn:
        for ay in aylar:
            try:
                ...
                conn.rollback()
                print(
                    f"  [BEKLEMEDE] {ingest.AY_ADLARI[ay]} 2018 işlenirken hata oluştu, "
