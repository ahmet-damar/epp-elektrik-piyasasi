...
# 2026-09-13 (ADIM 4, T2): "LPG" 2023 Ağustos-Aralık T2 tablosunda satır
# olarak görünüyor ama 12 ayın TAMAMINDA 2023'ün KENDİ üretim değeri
# (hedef_kolon) her zaman 0,00 MWh (yalnız 2022 karşılaştırma kolonunda
# küçük bir kalıntı var, "-100,00%" ile sıfırlanmış) — worker/parser.py'de
# hiç alias'ı yok (mimari karar: dokunulmadı), burada 'atla' sayılıyor.
# Güvence: t2_oku()'nun kendi Genel Toplam kontrolü LPG'nin gelecekte
# sıfır-olmayan bir değer taşıdığı bir ay varsa bunu OTOMATİK yakalar
# (toplam tutmaz, ValueError). T4-karşılığı (kurulu güç) tablosunda LPG/
# Motorin sütunu HİÇ YOK (12 ay kontrol edildi) — bu ekleme T4'ü etkilemez.
_KAYNAK_ATLA_ETIKETLERI = {"Genel Toplam", "Toplam", "İl Toplam", "LPG"}
...
def t2_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    ...
    uretim_envanteri.md) — dönem satırı '2023 HAZİRAN' gibi yıl-önce/
    tüm-büyük (2024/2025 ile AYNI). Kendi 'Genel Toplam' satırıyla
    aritmetik tutarlılık kontrol edilir (T4 ile AYNI ilke).

    2024'ün Bulgu I'i (3 satırlık bölünmüş başlık) 2023'ün 12 ayının
    TAMAMINDA GÖRÜLMEDİ (hepsi düz 2-satır) — yine de aynı dinamik
    atlama burada da uygulanıyor (ucuz bir önlem, yeni bir ay yeni bir
    varyant getirirse sessizce kırılmak yerine sağlam kalır)."""
    donem_satiri = [c.text.strip() for c in tbl.rows[0].cells]
    baslik_satiri = [c.text.strip() for c in tbl.rows[1].cells]
    hedef_kolon = hedef_donem_kolonu_bul(
...
def t3_oku(tbl, tarih_id: int) -> pd.DataFrame:
    ...
    oku()` ile sol+sağ blok birleştirilir. `_il_adi_temizle()` (2023'ün
    dipnot/inceltme düzeltmesi) burada da uygulanır. Eksik iller
    `uretim_mwh=0.0` ile AÇIKÇA tamamlanır, katı '81 il' assertion'ı
    YOK — asıl güvence kendi Genel Toplam'ıyla aritmetik tutarlılıktır."""
...
def isle_ay_uretim_geneli(
    ...
    *,
    ...
    """`fact_uretim_kaynak_geneli` + `fact_uretim_il_geneli`'ni 2023 Word
    aylarına doldurur — `word_2024.py:isle_ay_uretim_geneli()` ile
    BİREBİR AYNI desen.

    **2026-09-13 — Lisanslı (T2+T3) yüklenir, Lisanssız (T5/T6) KAPSAM
    DIŞI:** Bulgu H (`12_word_uretim_envanteri.md`) 2023'ün T6'sının
    (Lisanssız, il bazında) yıl-içi BÖLÜNDÜĞÜNÜ gösterdi — Ocak-Haziran
    var (eski başlıkla, "İhtiyaç Fazlası" DEĞİL), Temmuz-Aralık YOK. Bu
    turda T6'nın Ocak-Haziran kısmının GERÇEKTEN "Brüt Üretim" mi
    taşıdığı İNCELENMEDİ (gelecek bir turda 2018-2022 işlenirken
    değerlendirilecek) — güvenli/tutarlı taraf seçildi: 2023'ün TAMAMI
    (12/12 ay) için Lisanssız HER İKİ tabloda da simetrik olarak kapsam
    dışı bırakılıyor (2016-2017/2024/2025 ile AYNI ilke)."""
    dosya_adi = MANIFEST_2023[ay]
    yol = klasor / dosya_adi
    yil = 2023
    tarih_id = yil * 100 + ay
    ...
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2023-uretim-geneli-v1"
    ...
    ingest.audit_log_yaz(
        ...
        payload={
            "olay": "ingest_tamamlandi",
            "tarih_id": tarih_id,
            "kaynak": "word_2023_uretim_geneli",
            "tablolar": audit_tablolar,
            "not": "Yalnız Lisanslı (T2+T3) yüklendi - Lisanssız (T5/T6) "
            "kapsam dışı işaretlendi (Bulgu H, Karar 4'ün 2023'e genişlemesi, "
            "T6'nın yıl-içi bölünmüş olması yüzünden TÜM yıl simetrik olarak dışlandı).",
        },
    )
    ...
    for tablo_adi in ("fact_uretim_kaynak_geneli", "fact_uretim_il_geneli"):
        pipeline.kapsam_disi_isaretle(
            ...
            sebep="2023'te T6 (Lisanssız, il bazında) yıl-içi bölünmüş "
            "(Ocak-Haziran var/eski başlıkla, Temmuz-Aralık yok) ve "
            "Ocak-Haziran kısmının gerçekten 'Brüt Üretim' taşıdığı bu "
            "turda doğrulanmadı - bkz. 12_word_uretim_envanteri.md Bulgu "
            "H. Güvenli/tutarlı taraf: TÜM yıl simetrik kapsam dışı "
            "(2016-2017/2024/2025 Karar 4 ile aynı ilke).",
