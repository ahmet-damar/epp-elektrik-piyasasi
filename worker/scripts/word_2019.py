...
_KAYNAK_TAKMA_ADLAR: dict[str, str] = {
    "Güneş (Yoğunlş.)": "Güneş",
    # 2026-09-13 (ADIM 4, T2, Bulgu N): 2019'un T2'si (Lisanslı, kaynak
    # bazında) Ocak-Kasım'da Hidrolik'i "AKARSU" + "BARAJLI HİDROLİK" diye
    # İKİ AYRI satıra bölüyor (Aralık'ta tek "HİDROLİK" satırı — established
    # tekleşme). "AKARSU" worker/parser.py:KAYNAK_ESLEME'de zaten "Hidrolik"e
    # eşleniyor (["Akarsu","Barajlı","Hidrolik"]); ama "Barajlı Hidrolik" İKİ
    # kelime birleşik geldiği için (yalnız "Barajlı" DEĞİL) tam eşleşmiyor —
    # burada tek satırlık takma ad yeterli. t2_oku() bu iki satırı (parser.py
    # modül notundaki "Akarsu+Barajlı→Hidrolik ... toplanmalı" ilkesiyle
    # AYNI şekilde) TOPLAYIP tek "Hidrolik" satırına indiriyor — fact_uretim_
    # kaynak_geneli'nin UNIQUE(tarih_id, kaynak_id, lisans_id, batch_id)
    # kısıtı iki ayrı satırı KABUL ETMEZ.
...
def t2_oku(tbl, tarih_id: int, hedef_ay_yil: str) -> pd.DataFrame:
    ...
    dönemler-arası-karşılaştırma format (Bulgu B) — dönem satırı '2019
    OCAK' gibi yıl-önce/tüm-büyük (2020-2025 ile AYNI). Kendi 'Genel
    Toplam' satırıyla aritmetik tutarlılık kontrol edilir.

    **Bulgu N:** Ocak-Kasım 2019 Hidrolik'i "AKARSU" + "BARAJLI HİDROLİK"
    diye İKİ AYRI satıra bölüyor (Aralık tek "HİDROLİK" satırı) —
    `_KAYNAK_TAKMA_ADLAR`'daki takma ad ikisini de "Hidrolik"e eşliyor,
    burada bir `dict` biriktiricisiyle (T4'ün "Güneş (Fotovoltaik)"+
    "Güneş (Yoğunlş.)" TOPLAMA deseniyle AYNI ilke) TEK "Hidrolik"
    satırına indirgeniyor — aksi halde fact_uretim_kaynak_geneli'nin
...
def isle_ay_uretim_geneli(
    ...
    *,
    ...
    """`fact_uretim_kaynak_geneli` + `fact_uretim_il_geneli`'ni 2019 Word
    aylarına doldurur — `word_2020.py:isle_ay_uretim_geneli()` ile
    BİREBİR AYNI desen.
    ...
    **2026-09-13 — Lisanslı (T2+T3) yüklenir, Lisanssız (T5/T6) KAPSAM
    ...
    source_period = f"{yil}-{ay:02d}"
    parser_version = "word-2019-uretim-geneli-v1"
    ...
    ingest.audit_log_yaz(
        ...
        payload={
            "olay": "ingest_tamamlandi",
            "tarih_id": tarih_id,
            "kaynak": "word_2019_uretim_geneli",
            "tablolar": audit_tablolar,
            "not": "Yalnız Lisanslı (T2+T3) yüklendi - Lisanssız (T5/T6) "
            "kapsam dışı işaretlendi (Bulgu L: T6 hiçbir zaman Brüt "
            "Üretim değil, İhtiyaç Fazlası Satın Alınan). T2'de Ocak-Kasım "
            "'AKARSU'+'BARAJLI HİDROLİK' TOPLANARAK tek 'Hidrolik' satırına "
            "indirgendi (Bulgu N).",
        },
    )
    ...
    for tablo_adi in ("fact_uretim_kaynak_geneli", "fact_uretim_il_geneli"):
        pipeline.kapsam_disi_isaretle(
            ...
            sebep="2019'da T6 (Lisanssız, il bazında) var olsa da ÖLÇÜMLE "
